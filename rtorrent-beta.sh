...
if [ ! -d /var/www/rutor/plugins/erasedata/ ]; then # Check to see if plugins already installed (START)
...
case $plugin in
	...
	tar -zxvf _task-3.6.tar.gz -C $plugindir
	rm _task-3.6.tar.gz
	;;
	...
	url="http://srv.srve.io/src/plugins-3.6.tar.gz"
	...
	unpack="tar -zxvf $file -C /var/www/rutor/"
	fu_download
	apt-get -y install php5-geoip ffmpeg curl libzen0 libmediainfo0 mediainfo rar zip
	...
	sed -i "$ i\$topDirectory = '$homedir/';" /var/www/rutor/plugins/filemanager/conf.php
	;;
...
fi # Check to see if plugins already installed (END)
